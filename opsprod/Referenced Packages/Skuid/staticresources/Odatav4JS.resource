...
function handlerAccepts(handler, cType) {
    //SKUID - Because Jitterbit returns a text/xml contenttype instead of
    // application/xml, it fails to match metadata payloads here, so let's check for that
    if(handler.accept.indexOf('application/xml') >= 0) {
        if(cType.mediaType === 'text/xml') return true;
    }
...
function isJsonSerializableProperty(attribute) {
    ...
    // JLC: To successfully update reference fields, we must be able to serialize
    // attributes that contain @odata.bind.
    if (attribute.indexOf("@odata.bind") > -1 || attribute.indexOf("@odata.") == -1) {
        return true;
    }
...
function checkProperties(data, objectInfoType, baseURI, model, recognizeDates) {
  //JACKSON - This function was causing our payloads to register as a failure when the payload was just fine
  //So am short circuiting for now. Never seemed to work for us correctly anyway.
  return;
    for (var name in data) {
  	...
  	//If the base type and regular type does not contain the property (Navigation Property)
		//Should return <== Brought to you by: Jackson Stone
...
function addMetadataToJsonMinimalPayloadEntity(data, objectInfo, baseURI, model, recognizeDates) {
    ...
    //SKUID WAS HERE because of CORE-1349
    //We edited this line to allow keyType's withou key properties to still grab their
    //baseType props because Dynamics includes keys in non-baseType entities 
    //Also we prevent looping potiential below, because we trust no one!
    var loopDetection = {};
    while ((defined(keyType)) && (keyType.baseType !== undefined) && !loopDetection[keyType.baseType]) {
        loopDetection[keyType.baseType] = true;
        keyType = lookupEntityType(keyType.baseType, model);
    }
    //This made it very hard to mock tests
		// so I changed it. @author: skuid, jackson stone.
    if (keyType && keyType.key !== undefined) {
...
/** Given an expected alias prefix, removes it from a full name.
 * @param {String} alias - Expected alias.
 * @param {String} fullName - Full name in ’ns’.’name’ form.
 * @returns {String} The local name, null if it isn’t found in the expected namespace.
 */
function removeAlias(alias, fullName) {
    if (fullName.indexOf(alias) === 0 && fullName.charAt(alias.length) === ".") {
	return fullName.substr(alias.length + 1);
    }
...
function lookupInSchema(name, schema, kind) {
	...
	if (!nameOnly) nameOnly = removeAlias(schema.alias, name);
...
exports.DomStore      = require('./store/dom.js');
exports.IndexedDBStore = require('./store/indexeddb.js');
exports.MemoryStore    = require('./store/memory.js');


exports.mechanisms = {
    indexeddb: exports.IndexedDBStore,
    dom: exports.DomStore,
    memory: exports.MemoryStore
};
